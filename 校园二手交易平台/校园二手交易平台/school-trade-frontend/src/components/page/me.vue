...
<script>
    ...
    export default {
        ...
        methods: {
                ...
                this.$api.getAllIdleItem().then(res => {
                    ...
                    if (res.status_code === 1) {
                        for (let i = 0; i < res.data.length; i++) {
                            ...
                            //根据闲置商品状态分类：1 --上架，存入dataList[0]；2 --下架，存入this.dataList[1]
            ...
            //详情跳转，当选择我买或我卖的标签，跳转到订单详情；其余则跳转到闲置商品详情页
                ...
                this.$confirm('是否确认？', '提示', {
                    ...
                    if(activeName==='1'){
                        this.$api.updateIdleItem({
                            ...
                            //若后端返回的成功请求
                            if(res.status_code===1){
                                this.dataList[0].splice(index,1); //从“我发布的”中移除该闲置商品
                                item.idleStatus=2;
                                this.dataList[1].unshift(item);  //添加到“我下架的”中
                            }else {
                                this.$message.error(res.msg)
                            }
                        });
                    }else if(activeName==='2'){    //在“我下架的”中选择彻底删除下架商品
                        this.$api.updateIdleItem({
                            id:item.id,
                            idleStatus:0  //状态改为删除
                        }).then(res=>{
                            console.log(res);
                            if(res.status_code===1){
                                this.dataList[1].splice(index,1); //从“我下架的”中移除
                            }else {
                                this.$message.error(res.msg)
                            }
                        });
                    }else if(activeName==='3'){  //从“我收藏的”中取消收藏
                        this.$api.deleteFavorite({
                            ...
                            if(res.status_code===1){
                                ...
                                this.dataList[2].splice(index,1); //从“我收藏的”中移除
                            }else {
                                this.$message.error(res.msg)
                            }
                        }).catch(e=>{
            ...
            fileHandleSuccess(response, file, fileList) {
                ...
                this.imgFileList = []; //清空文件列表
                this.$api.updateUserPublicInfo({   //更新用户头像
                    avatar: imgUrl
                }).then(res => {
                    console.log(res);
                    this.userInfo.avatar = imgUrl;
                    this.$globalData.userInfo.avatar = imgUrl; //同步全局数据
                })
            },
            //更新地址
            update(data) {
